...
func set(c *Cache, ns, key uint64, value Value, charge int, relf func()) *Handle {
	return c.Get(ns, key, func() (int, Value) {
		if relf != nil {
			return charge, releaserFunc{fn: relf, value: value}
		}
		return charge, value
	})
}
...
func TestCacheMap(t *testing.T) {
	...
	if testing.Short() {
		params = []cacheMapTestParams{
			{nObjects: 1000, nHandles: 100, concurrent: 20, repeat: 3},
			{nObjects: 10000, nHandles: 300, concurrent: 50, repeat: 10},
		}
	} else {
		params = []cacheMapTestParams{
			{nObjects: 10000, nHandles: 400, concurrent: 50, repeat: 3},
			{nObjects: 100000, nHandles: 1000, concurrent: 100, repeat: 10},
		}
	}
