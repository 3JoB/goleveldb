...
func (kv *KeyValue) Put(key, value []byte) {
	...
	kv.entries = append(kv.entries, KeyValueEntry{key: key, value: value})
...
func (kv *KeyValue) PutU(key, value []byte) bool {
	if i, exist := kv.Get(key); !exist {
		if i < kv.Len() {
			...
			kv.entries[i] = KeyValueEntry{key: key, value: value}
		} else {
			kv.entries = append(kv.entries, KeyValueEntry{key: key, value: value})
		}
		kv.nbytes += len(key) + len(value)
...
func (kv KeyValue) Clone() KeyValue {
	return KeyValue{entries: append([]KeyValueEntry{}, kv.entries...), nbytes: kv.nbytes}
}
...
func (kv KeyValue) Slice(start, limit int) KeyValue {
	...
	return KeyValue{entries: append([]KeyValueEntry{}, kv.entries[start:limit]...), nbytes: kv.nbytes}
}
