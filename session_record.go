...
func (p *sessionRecord) addCompPtr(level int, ikey internalKey) {
	...
	p.compPtrs = append(p.compPtrs, cpRecord{level: level, ikey: ikey})
}
...
func (p *sessionRecord) addTable(level int, num, size int64, imin, imax internalKey) {
	...
	p.addedTables = append(p.addedTables, atRecord{level: level, num: num, size: size, imin: imin, imax: imax})
}
...
func (p *sessionRecord) delTable(level int, num int64) {
	...
	p.deletedTables = append(p.deletedTables, dtRecord{level: level, num: num})
}
...
func (p *sessionRecord) readUvarintMayEOF(field string, r io.ByteReader, mayEOF bool) uint64 {
	...
	if err != nil {
		if err == io.ErrUnexpectedEOF || (!mayEOF && err == io.EOF) {
			p.err = errors.NewErrCorrupted(storage.FileDesc{}, &ErrManifestCorrupted{Field: field, Reason: "short read"})
		} else if strings.HasPrefix(err.Error(), "binary:") {
			p.err = errors.NewErrCorrupted(storage.FileDesc{}, &ErrManifestCorrupted{Field: field, Reason: err.Error()})
		} else {
			p.err = err
		}
		return 0
...
func (p *sessionRecord) readVarint(field string, r io.ByteReader) int64 {
	...
	if x < 0 {
		p.err = errors.NewErrCorrupted(storage.FileDesc{}, &ErrManifestCorrupted{Field: field, Reason: "invalid negative value"})
	}
	return x
}
