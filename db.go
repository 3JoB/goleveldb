...
func (db *DB) recoverJournal() error {
	...
	if len(fds) > 0 {
		...
		for _, fd := range fds {
			...
			if jr == nil {
				jr = journal.NewReader(fr, dropper{s: db.s, fd: fd}, strict, checksum)
			} else {
				// Ignore the error here
				_ = jr.Reset(fr, dropper{s: db.s, fd: fd}, strict, checksum)
			}
	...
	if len(fds) > 0 {
		...
		for _, fd := range fds {
			...
			if jr == nil {
				...
				if err := jr.Reset(fr, dropper{s: db.s, fd: fd}, strict, checksum); err != nil {
					return err
				}
			}
